...
4. **`do_cert_check`**: reloads config, then either always calls `run_certbot` (for domain/force triggers) or calls `cert_needs_renewal` first (for scheduled checks). `cert_needs_renewal` uses `openssl x509 -checkend`.
5. **`run_certbot`**: selects `--expand` (domain added) or `--force-renewal` (domain removed, both, or force) flags, runs certbot with `--dns-cloudflare`, and resets `FORCE_RENEW=false` in `.env` via `sed -i` after success.

### `deploy-hook.sh` — certbot post-renewal hook
Called by certbot via `--deploy-hook` after every successful renewal. When `SYNOLOGY_DEPLOY=true`: authenticates with DSM (`/webapi/auth.cgi`), uploads `privkey.pem` / `cert.pem` / `chain.pem` via multipart POST to `/webapi/entry.cgi`, sets cert as default, then logs out.
...
### `Dockerfile`
Alpine 3.23 base. Installs: `python3`, `certbot`, `certbot-dns-cloudflare` (via pip), `inotify-tools`, `jq`, `openssl`, `bash`, `curl`, `tzdata`. No build arguments.
